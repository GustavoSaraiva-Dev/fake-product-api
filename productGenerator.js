const generateProducts =
	(arrayLength, numberOfImage, descriptionLength) =>
	...
	(minCost, maxCost) => {
		...
		for (let i = 0; i < a.length; i++) {
			...
			const randomCategory =
				Math.floor(Math.random() * (+maxCat - +minCat)) + +minCat;
			const randomStock = Math.floor((Math.random() * 1000) % 2);
			const randomPrice = Math.random() * (+maxPrice - +minPrice) + +minPrice;

      //generate random description by number of words from parameter descriptionLength
      ...
      function generateWords(n){
        ...
        return words.join(" ");
			...
			for (let index = 0; index < numberOfImage; index++) {
				imgArray.push({
					id: index + 1,
					url: `https://picsum.photos/id/${Math.floor(Math.random() * 1000)}/800/600.webp`,
				});
			}
			a[i] = {
				productId: 1000 + i + 1,
				...
				description: generateWords(descriptionLength),
				images: imgArray,
...
// sample execution
generateProducts(20, 6, 8)(1, 8)(5, 289);
